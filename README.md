...
1. **Build the Docker Image:**
   ```bash
   docker build -t gitats .
   ```
2. **Run the Container:**
   ```bash
   docker run --env-file .env -p 8080:8080 gitats
   ```
   Or mount `env.yaml`:
   ```bash
   docker run -v $(pwd)/env.yaml:/app/env.yaml -p 8080:8080 gitats
   ```
...
gitats generates structured JSON output optimized for ATS systems and easy integration into resume templates. The schema includes:
...
gitats uses a modular, scalable architecture:
